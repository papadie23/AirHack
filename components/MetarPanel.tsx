...
import type { MetarResponse, MetarCloud, MetarWind, MetarTrend, MetarProvider } from "@/lib/metar";
import {
  flightCategoryColor,
  windDirectionLabel,
  cloudCoverLabel,
...
interface Props {
  ...
  onProviderChange: (p: MetarProvider) => void;
}

const PROVIDERS: { id: MetarProvider; label: string; sublabel: string }[] = [
  { id: "noaa",    label: "NOAA",    sublabel: "Official / Live" },
  { id: "checkwx", label: "CheckWX", sublabel: "API key req." },
  { id: "fixture", label: "Fixture", sublabel: "Demo" },
];
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-[#07090f]">
      ...
      <div className="px-5 py-3 border-b border-white/5 flex items-center gap-2 shrink-0">
        ...
        {metar?.warning && (
          <span className="ml-auto text-[10px] text-amber-400 font-mono truncate max-w-[180px]" title={metar.warning}>
            ⚠ {metar.warning.slice(0, 40)}{metar.warning.length > 40 ? "…" : ""}
          </span>
        )}
        {metar?.fromFixture && !metar.warning && (
          <span className="ml-auto text-[10px] font-mono text-yellow-600 bg-yellow-900/20 border border-yellow-800/30 px-2 py-0.5 rounded">
            FIXTURE
          </span>
        )}
      </div>
...
function FlightCategoryBadge({ category }: { category: "VFR" | "MVFR" | "IFR" | "LIFR" }) {
  const c = flightCategoryColor(category);
  return (
    <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded border ${c.bg} ${c.text} ${c.border}`}>
      {category}
    </span>
  );
}
...
function VisibilityDisplay({ meters, unlimited, phenomena }: {
  meters: number; unlimited: boolean; phenomena: string[];
}) {
  return (
    <MetarCell label="Visibility">
      ...
      {phenomena.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-1">
          {phenomena.map((p) => (
            <span key={p} className="text-[10px] font-mono bg-yellow-900/40 border border-yellow-600/40 text-yellow-300 px-1.5 py-0.5 rounded">
              {p}
            </span>
          ))}
        </div>
...
function TrendBlock({ trend }: { trend: MetarTrend }) {
  const isWarning = trend.phenomena.includes("TSRA") || trend.phenomena.includes("TS") ||
  ...
  return (
    <section className="px-5 py-4">
      ...
      <div className={`mt-2 rounded-xl border p-3 font-mono text-xs ${cls}`}>
        <div className="flex items-center gap-2 mb-2">
          <span className="font-bold text-sm">{trend.type}</span>
          {isWarning && <span>⚠</span>}
          {trend.phenomena.map((p) => (
            <span key={p} className="px-1.5 py-0.5 rounded bg-black/30 text-[10px]">{p}</span>
          ))}
        </div>
        <div className="flex flex-col gap-1">
          {trend.wind && (
            <TrendRow label="Wind">
              {trend.wind.isVariable ? "VRB" : `${trend.wind.directionDeg.toString().padStart(3, "0")}°`}{" "}
              {trend.wind.speedKt} KT
              {trend.wind.gustKt != null && <span className="text-amber-400"> G{trend.wind.gustKt}</span>}
            </TrendRow>
          )}
          {trend.visibility && (
            <TrendRow label="Vis">{formatVisibility(trend.visibility.meters, trend.visibility.unlimited)}</TrendRow>
          )}
          {trend.clouds.map((c, i) => (
            <TrendRow key={i} label="Cloud">
              <span className={c.type === "CB" ? "text-amber-300 font-bold" : ""}>
                {c.cover}{Math.round(c.baseFt / 100).toString().padStart(3, "0")}{c.type ?? ""}
              </span>
            </TrendRow>
          ))}
        </div>
...
function RawMetarBox({ raw }: { raw: string }) {
  return (
    <section className="px-5 py-4">
...
function MetarCell({ label, children }: { label: string; children: React.ReactNode }) {
  return (
